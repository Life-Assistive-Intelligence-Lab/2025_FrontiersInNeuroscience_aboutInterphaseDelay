function Result_Fig2FHJ_control_comparePeakPlot

    rpwd = pwd;

    Func_plot_EachRow(1,1:3) % Figure 2F
    Func_plot_EachRow(2,4:10) % Figure 2H
    Func_plot_EachRow(3,11:40) % Figure 2J
...
function Func_plot_EachRow(figIdx,bin_range)
    ...
    panel_idx = {'F','H','J'};
    Func_saveData_toCSV(data_forStatistical, dataName_forStatistical,['Fig2' char(panel_idx(figIdx)) '_control_ComparePeak_for' num2str(bin_range(1)-1) 'to' num2str(bin_range(end)) 'msec'])
...
function y_list = func_make_data(tissueSlice_list,stim_name,base_name,bin_range)
    ...
    for forEach_slice = 1:max(size(tissueSlice_list))
        ...
        y_list = [y_list max(lineprofile)/max(lineprofile_forNrm)];
    end

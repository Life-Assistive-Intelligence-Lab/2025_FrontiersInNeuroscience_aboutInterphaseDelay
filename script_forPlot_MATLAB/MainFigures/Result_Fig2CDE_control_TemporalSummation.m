function Result_Fig2CDE_control_TemporalSummation
    rpwd = pwd;
...
function Func_plot_EachRow(plot_row,bin_range)
    ...
    [~,y_list] = func_make_data(tissueSlice_list,'10uA_Cathode','10uA_Cathode',bin_range);
    maxValue_cathode = max(mean(y_list,1));

    for forEach_stim = 1:max(size(ipd_labelList))
    
        stim_name = char(ipd_labelList(forEach_stim));
        [x,y_list] = func_make_data(tissueSlice_list,stim_name,'10uA_Cathode',bin_range);
        ...
        Func_Errorbar_VerAreaImaging(-x,mean(y_list,1),std(y_list,1),color_code);
        xlim(-x([end 1]));
        ylim([-0.25 1.3]*maxValue_cathode);

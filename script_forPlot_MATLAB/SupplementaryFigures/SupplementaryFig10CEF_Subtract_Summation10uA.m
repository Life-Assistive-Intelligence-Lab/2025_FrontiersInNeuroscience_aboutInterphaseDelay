function SupplementaryFig10CEF_Subtract_Summation10uA
    ...
    stim_labelList_forPlot = {'IPDis___0', ...
                      'IPDis_100', ...
                      'IPDis_200', ...
                      'IPDis_300', ...
                      'IPDis_400', ...
    ...
    [~,y] = func_make_data(tissueSlice_list,char(stim_labelList(end)),char(stim_labelList(end)));
    ...
    for forEach_stim = 1:max(size(stim_labelList))
        ...
        [x,y] = func_make_data(tissueSlice_list,stim_name,char(stim_labelList(end)));
        ...
        dataName_forStatistical(end+1) = {char(stim_labelList_forPlot(forEach_stim))};
    end



    dataBefore_forStatistical = dataBefore_forStatistical.';
    Func_saveData_toCSV(dataBefore_forStatistical, dataName_forStatistical,'SupplementaryFig10E_Subtract_Summation_10uABefore')

    dataAfter_forStatistical = dataAfter_forStatistical.';
    Func_saveData_toCSV(dataAfter_forStatistical, dataName_forStatistical,'SupplementaryFig10F_Subtract_Summation_10uAAfter')
...
function [indx_list,data_list] = func_make_data(tissueSlice_list,stim_name,base_name)
    ...
    for forEach_slice = 1:max(size(tissueSlice_list))
        [data, information] = Func_data_loader(tissueSlice_list(forEach_slice),'Control-DAP5_DNQX',stim_name);

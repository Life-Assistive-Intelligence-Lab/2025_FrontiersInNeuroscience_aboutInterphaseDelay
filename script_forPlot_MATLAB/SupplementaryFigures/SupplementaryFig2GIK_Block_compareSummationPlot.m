function SupplementaryFig2GIK_Block_compareSummationPlot

    rpwd = pwd;

    Func_plot_EachRow(1,1:3) % Supplementary Figure 2G
    Func_plot_EachRow(2,4:10) % Supplementary Figure 2I
    Func_plot_EachRow(3,11:40) % Supplementary Figure 2K
...
function Func_plot_EachRow(figIdx,bin_range)
    ...
    panel_idx = {'G','I','K'};
    Func_saveData_toCSV(data_forStatistical, dataName_forStatistical,['SupplementaryFig2' char(panel_idx(figIdx)) '_Block_CompareSummation_for' num2str(bin_range(1)-1) 'to' num2str(bin_range(end)) 'msec'])
...
function data_list = func_make_data(tissueSlice_list,stim_name,base_name,bin_range)
    ...
    for forEach_slice = 1:max(size(tissueSlice_list))
        ...
        data_list = [data_list sum(lineprofile)/sum(lineprofile_forBase)];
    end

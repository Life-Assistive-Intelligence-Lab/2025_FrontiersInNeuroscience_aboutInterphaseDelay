% Plot individual values, mean, and s.e.m. used for statistical analysis. 
% function Func_BarPlot_forFewSample(x,y,x_size,color_code,marker_size)
    ...
    % Marker - Each value
    marker_list = {'o','v','^','<','>','square'};
    marker_Fixsize = [1.25 1 1 1 1 1.7];

    for forEach_idx = 1:max(size(y))
        modulatedX = x + (forEach_idx-1-(max(size(y))-1)/2) * x_size /6;
